...
def generate(ftxt, mode, argument=False):
    ...
    for idx, name, padding in types:
        ...
        with h5py.File('/home/admin01/workspace/deep_landmark/dataset/train/3_%s/%s.h5'%(name, mode), 'w') as h5:
            h5['data'] = patches.astype(np.float32)
            h5['landmark'] = landmarks.astype(np.float32)
        with open('/home/admin01/workspace/deep_landmark/dataset/train/3_%s/%s.txt'%(name, mode), 'w') as fd:
            fd.write('/home/admin01/workspace/deep_landmark/dataset/train/3_%s/%s.h5'%(name, mode))
