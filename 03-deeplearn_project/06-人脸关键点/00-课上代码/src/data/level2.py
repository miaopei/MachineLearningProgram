...
types = [(0, 'LE1', 0.16),
         ...
         (4, 'RM2', 0.18),]
for t in types:
    d = '/home/tyd/下载/deep_landmark/mydataset/mytrain/2_%s' % t[1]
    createDir(d)
...
def generate(ftxt, mode, argument=False):
    ...
    for idx, name, padding in types:
        ...
        with h5py.File('/home/tyd/下载/deep_landmark/mydataset/mytrain/2_%s/%s.h5'%(name, mode), 'w') as h5:
            h5['data'] = patches.astype(np.float32)
            h5['landmark'] = landmarks.astype(np.float32)
        with open('/home/tyd/下载/deep_landmark/mydataset/mytrain/2_%s/%s.txt'%(name, mode), 'w') as fd:
            fd.write('/home/tyd/下载/deep_landmark/mydataset/mytrain/2_%s/%s.h5'%(name, mode))
